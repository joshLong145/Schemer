...
pub fn std_env() -> HashMap<String, ProcedureFn> {
    ...
    env.insert(
        ...
        Box::new(|args, _| {
            ...
            // A non-empty list is also a pair in R7RS
...
fn list_to_vec(val: &Value) -> Result<Vec<Value>, String> {
    ...
    loop {
        match current {
            Value::Nil => break,
            Value::List(list) => {
                // SchemeList has a to_vec method
...
// Helper: convert Vec to Value list (using SchemeList for proper lists)
...
fn list_length(val: &Value) -> Result<i64, String> {
    match val {
        Value::Nil => Ok(0),
        Value::List(list) => Ok(list.length() as i64),
        Value::Pair(_) => {
            let mut count = 0i64;
            let mut current = val.clone();
            loop {
...
// Helper: deep equality check that handles both List and Pair representations
fn values_equal(a: &Value, b: &Value) -> bool {
    match (a, b) {
        ...
        // List/Pair comparisons - normalize to Vec and compare
        (Value::List(_), Value::List(_))
        | (Value::Pair(_), Value::Pair(_))
        | (Value::List(_), Value::Pair(_))
        | (Value::Pair(_), Value::List(_)) => match (list_to_vec(a), list_to_vec(b)) {
            (Ok(vec_a), Ok(vec_b)) => {
